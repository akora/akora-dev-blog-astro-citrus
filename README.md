...
11. [Webmention Setup](#webmention-setup)
12. [Deploy](#deploy)
...
- Modify file `src/styles/global.css` with your own light and dark styles.
  - You can also modify the theme(s) for markdown code blocks generated by [Rehype Pretty Code](https://rehype-pretty.pages.dev/). Astro Citrus has both a dark (rose-pine) and light (rose-pine-dawn) theme, which can be found in `src/site.config.ts`. You can find more themes and options on the [Shiki website](https://shiki.matsu.io/).
...
- OG Image:
  ...
  - You can also create your own OG images and skip Satori's automatic generation by adding an `ogImage` property in the frontmatter with a link to the asset. An example can be found in `src/content/post/social-image.md`. More info on frontmatter can be found in the [frontmatter section](#post-frontmatter).
- Optional:
...
If you have a template repository, you can add this template as a remote, as [discussed on Stack Overflow](https://stackoverflow.com/questions/56577184/github-pull-changes-from-a-template-repository).
...
You can reduce the initial CSS payload of your site, as demonstrated in [this pull request](https://github.com/artemkutsan/astro-citrus/pull/145#issue-1943779868), by lazy-loading the web component's styles.

## Webmention Setup

This theme supports [Webmentions](https://webmention.io/) to receive notifications when other websites link to your content. To enable this feature, follow these steps:

1. **Sign up on webmention.io**: Create a free account at [webmention.io](https://webmention.io).

2. **Add Your Domain**: In your webmention.io dashboard, go to **Settings > Sites** and add your website's domain (e.g., `your-domain.com`).

3. **Set Environment Variables**: In your deployment provider (e.g., Netlify, Vercel), set the following environment variables:

   - `WEBMENTION_API_KEY`: Your secret API key from the webmention.io dashboard.
   - `PUBLIC_WEBMENTION_URL`: `https://webmention.io/YOUR_DOMAIN/webmention` (replace `YOUR_DOMAIN` with your actual domain).
   - `PUBLIC_WEBMENTION_PINGBACK`: `https://webmention.io/YOUR_DOMAIN/xmlrpc` (replace `YOUR_DOMAIN` with your actual domain).

4. **Configure Secret Scanning (for Netlify)**: Netlify's build process may fail due to its secret scanning feature detecting the public variables. To fix this, add `SECRETS_SCAN_OMIT_KEYS` to your environment variables in the Netlify UI with the following value:

   ```text
   PUBLIC_WEBMENTION_URL,PUBLIC_WEBMENTION_PINGBACK
   ```

Once configured, your site will be ready to receive webmentions. Note that this theme does not include a component to _display_ webmentions by default.
